...
namespace LucaasBot.Music
{
    public class SoundCloudApiService
    {
        ...
        public async Task<SoundCloudVideo> ResolveVideoAsync(string url)
        {
            ...
            string response = await client.GetStringAsync($"https://scapi.nadeko.bot/resolve?url={url}").ConfigureAwait(false);
        ...
        public async Task<SoundCloudVideo> GetVideoByQueryAsync(string query)
        {
            ...
            var response = await client.GetStringAsync(new Uri($"https://scapi.nadeko.bot/tracks?q={Uri.EscapeDataString(query)}")).ConfigureAwait(false);
    ...
    public class SoundCloudVideo
    {
        ...
        public Task<SongInfo> GetSongInfo() =>
            Task.FromResult(new SongInfo
            {
                ...
                Query = this.TrackLink,
                Thumbnail = this.ArtworkUrl,
                TotalTime = TimeSpan.FromMilliseconds(this.Duration)
            });
    }
