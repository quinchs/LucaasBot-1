    ...
    public class MusicPlayer
    {
        ...
        private async void PlayerLoop()
        {
            while (!Exited)
            {
                ...
                if (data.Song != null)
                {
                    Logger.Log("Starting", Severity.Music);
                    ...
                    try
                    {
                        ...
                        if (ac == null)
                        {
                            Logger.Log("Can't join", Severity.Music);
                        ...
                        Logger.Log("Created pcm stream", Severity.Music);
                    ...
                    finally
                    {
                        ...
                        if (_bytesSent == 0 && !cancel)
                        {
                            ...
                            Logger.Log("Song removed because it can't play", Severity.Music);
                        }
                    }
                    try
                    {
                        ...
                        if (!manualIndex && (!RepeatCurrentSong || manualSkip))
                        {
                            if (Shuffle)
                            {
                                Logger.Log("Random song", Severity.Music);
                            ...
                            else
                            {
                                ...
                                if (queueCount - 1 == data.Index && Autoplay && data.Song?.ProviderType == MusicType.YouTube)
                                {
                                    try
                                    {
                                        Logger.Log("Loading related song", Severity.Music);
                                        await _musicService.TryQueueRelatedSongAsync(data.Song, TextChannel, VoiceChannel).ConfigureAwait(false);
                                        if (!AutoDelete)
                                            Queue.Next();
                                    }
                                    catch
                                    {
                                        Logger.Warn("Loading related song failed.", Severity.Music);
                                    }
                                }
                                else if (FairPlay)
                                {
                                    lock (locker)
                                    {
                                        Logger.Log("Next fair song", Severity.Music);
                                ...
                                {
                                    Logger.Log("Stopping because repeatplaylist is disabled", Severity.Music);
                                    lock (locker)
                                    {
                                        Stop();
                                    }
                    ...
                    {
                        Logger.Error(ex, Severity.Music);
                    }
                }
                do
                {
                ...
                try
                {
                    try
                    {
                        ...
                        if (t != null)
                        {
                            ...
                            Logger.Log("Stopping audio client", Severity.Music);
                            await t.ConfigureAwait(false);

                            Logger.Log("Disposing audio client", Severity.Music);
        ...
        {
            Logger.Log("Destroying", Severity.Music);
            lock (locker)
            {
                Stop();
                Exited = true;
